def grad_cam(model, image, category_index, layer_name):
    ...
    cam5 = cv2.resize(np.float32(cam[4]), (112, 112), cv2.INTER_AREA)
    cam5 = np.maximum(cam5, 0)
    heatmap5 = cam5 / np.max(cam5)

    cam6 = cv2.resize(np.float32(cam[5]), (112, 112), cv2.INTER_AREA)
    cam6 = np.maximum(cam6, 0)
    heatmap6 = cam6 / np.max(cam6)

    cam7 = cv2.resize(np.float32(cam[6]), (112, 112), cv2.INTER_AREA)
    cam7 = np.maximum(cam7, 0)
    heatmap7 = cam7 / np.max(cam7)

    cam8 = cv2.resize(np.float32(cam[7]), (112, 112), cv2.INTER_AREA)
...
fol = 'gradcam_8_avg' + str(folder_name)
